...
// 创建一个 updateListener
const slashListener = EditorView.updateListener.of((update) => {
  console.log("xxxxxxx");
  ...
  if (update.docChanged) {
    ...
    changes.iterChanges((fromA, toA, fromB, toB, inserted) => {
      ...
      if (insertedText.includes('/')) {
        ...
        // 获取插入 '/' 字符的位置
        const insertPos = fromB + insertedText.indexOf('/');

        // 插入 '-' 字符
        update.view.dispatch({
          changes: {
            from: insertPos + 1, // 在 '/' 后插入
            insert: '-',
          }
        });
      }
...
function useCodeMirrorScope(view: EditorView) {
  ...
  useEffect(() => {
    if (currentDoc) {
      ...
      // 实例化的对象
      // @ts-ignore
      window.cInstantiateEditorView = view;
      // @ts-ignore
      window.cInstantiateEditorState = state;

      // 操作时可能需要用到的对象
      // @ts-ignore 
      window.cStateEffect = StateEffect;
      // @ts-ignore 
      window.cEditorView = EditorView;
      // @ts-ignore 
      window.cEditorState = EditorState;

      function createIFrame() {
        const iframe = document.createElement('iframe');
        iframe.id = "CIFrame"
        iframe.src = "http://localhost/project";
        iframe.width = "300px";
        iframe.height = "500px";
